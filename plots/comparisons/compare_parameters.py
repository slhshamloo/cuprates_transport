...
def calculate_free():
    ...
    # vary_value("free", "gamma_k", "gamma_k", [0, 10, 100, 1000],
    #            np.linspace(0, 20, 100), params, bandObject)
    vary_value("free", "power", "power", [2, 6, 12, 18],
               np.linspace(0, 20, 100), params, bandObject)

    # params["gamma_k"] = 0
    # vary_value("free_iso", "Bamp", "B", [0, 500, 1000, 1500, 2000],
    #            np.linspace(-20, 20, 100), params, bandObject)
    # vary_value("free_iso", "gamma_0", "gamma_0", [10, 15, 20, 25],
    #            np.linspace(0, 20, 100), params, bandObject)
    # params["gamma_k"] = 20
    # vary_value("free_aniso", "Bamp", "B", [0, 500, 1000, 1500, 2000],
    #            np.linspace(-20, 20, 100), params, bandObject)
    # vary_value("free_aniso", "gamma_0", "gamma_0", [10, 15, 20, 25],
    #            np.linspace(0, 20, 100), params, bandObject)
...
def calculate_lsco():
    ...
    # vary_value("NdLSCO", "gamma_k", "gamma_k", [0, 10, 100, 1000],
    #            np.linspace(0, 20, 100), params, bandObject)
    vary_value("NdLSCO", "power", "power", [2, 6, 12, 18],
               np.linspace(0, 20, 100), params, bandObject)

    params["gamma_k"] = 0
    # vary_value("NdLSCO_iso", "Bamp", "B", [0, 50, 100, 150, 200],
    #            np.linspace(-20, 20, 50), params, bandObject)
    vary_value("NdLSCO_iso", "gamma_0", "gamma_0", [10, 15, 20, 25],
               np.linspace(0, 20, 100), params, bandObject)
    params["gamma_k"] = 20
    # vary_value("NdLSCO_aniso", "Bamp", "B", [0, 50, 100, 150, 200],
    #             np.linspace(-20, 20, 50), params, bandObject)
    vary_value("NdLSCO_aniso", "gamma_0", "gamma_0", [10, 15, 20, 25],
               np.linspace(0, 20, 100), params, bandObject)
...
def load_data(band, value_label, value):
    return np.loadtxt(
        f"user_data/vary/{band}_{value_label}_{value}.csv",
...
def plot_free_vary_gamma_k():
    ...
    plot_data(axs, "free", "gamma_k", "\\Gamma_k", [0, 10, 100, 1000],
...
def plot_lsco_vary_gamma_k():
    fig, axs = generate_figure_single(figsize=(10, 8))
    fig.suptitle("LSCO, No Magnetic Field", y=0.94)
    plot_data(axs, "NdLSCO", "gamma_k", "\\Gamma_k", [0, 10, 100, 1000],
              convert_to_lr=False, lw=2)
    axs[0].legend(loc="upper right", fontsize=20, frameon=False,
                  ncols=2, columnspacing=1.0)
    axs[1].text(0.03, 0.9, fr"$\Gamma_0 = 10$" + "\n" + fr"$\nu = 6$",
...
def plot_free_iso_vs_aniso_vary_B():
    ...
    plot_data(axs[:, 0], "free_iso", "B", "B",
              [0, 500, 1000, 1500, 2000], lw=2)
    plot_data(axs[:, 1], "free_aniso", "B", "B",
              [0, 500, 1000, 1500, 2000], lw=2)
...
def plot_free_iso_vs_aniso_vary_gamma_0():
...
def plot_lsco_iso_vs_aniso_vary_B():
    fig, axs = generate_figure_double(figsize=(12, 8), sigma_substript="ll/rr")
    axs[0, 0].set_title("Nd-LSCO, Isotropic", pad=10)
    axs[0, 1].set_title("Nd-LSCO, Anisotropic", pad=10)

    plot_data(axs[:, 0], "NdLSCO_iso", "B", "B",
              [0, 50, 100, 150, 200], lw=2)
    plot_data(axs[:, 1], "NdLSCO_aniso", "B", "B",
              [0, 50, 100, 150, 200], lw=2)
    ...
    plt.tight_layout()
    fig.savefig("user_plots/comparisons/NdLSCO_iso_vs_aniso_vary_field.pdf")
    plt.show()


def plot_lsco_iso_vs_aniso_vary_gamma_0():
    fig, axs = generate_figure_double(figsize=(12, 8))
    axs[0, 0].set_title("Nd-LSCO, Isotropic", pad=10)
    axs[0, 1].set_title("Nd-LSCO, Anisotropic", pad=10)
