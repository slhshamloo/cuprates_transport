...
filename = f"sigmaxx__w_0_{wmax}__B_{B}"

data = np.loadtxt(f"{datapath}/{filename}.dat", dtype="complex", comments="#", delimiter=',')

# data = np.linspace([0,15,99],[10,20,40],100)
...
Im_sigmaxx_Drude = np.imag(data[:,2])
...
# color = '#00EFD0'
linesCalc = []
linesCalc.append(axs[0].plot(x_omega, Re_sigmaxx_Chambers, label = r"Chambers"))
linesCalc.append(axs[1].plot(x_omega, Im_sigmaxx_Chambers, label = r"Chambers"))

linesRef = []
linesRef.append(axs[0].plot(x_omega, Re_sigmaxx_Drude, label = r"Drude"))
linesRef.append(axs[1].plot(x_omega, Im_sigmaxx_Drude, label = r"Drude"))


for l in linesCalc:
  plt.setp(l, ls ="-", c = color, lw = 2, marker = "", mfc = color, ms = 7, mec = color, mew= 2)

for l in linesRef:
  plt.setp(l, ls ="--", c = colorDrude, lw = 2, marker = "", mfc = colorDrude, ms = 7, mec = colorDrude, mew= 2)

  ######################################################
# hline for imaginary part

  ######################################################
axs[0].legend(loc = 3, fontsize = 16, frameon = False, numpoints=1, markerscale = 1, handletextpad=0.5)
